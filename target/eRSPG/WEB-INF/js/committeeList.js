$(function() {
    ...
    /*
        This will return a list of committee members to be displayed
        on the /eRSPG/chair/proposal for the Modal Dialog view.
    */
    fetchCommittee1();
...
const committeeTableBuilder = (committeeList) => {
 ...
 /*
    fills table with current committee members for the
    Modeal Dialog View on the  /eRSPG/chair/proposal page.
  */
const committeeTableBuilder1 = (committeeList) => {
    const tableBodyElm = $('#committee-members');
    if (jQuery.isArray(committeeList)) {
        committeeList.map((p) => {
            tableBodyElm.append(`
            <tr>
                <td><input type="checkbox" name="committee-member"/></td>
                <td data-label="First Name">${p.firstName}</td>
                <td data-label="Last Name" class="last">${p.lastName}</td>
                <td data-label="Email">${p.email}</td>
...
// fetches all committee members
const fetchCommittee1 = () => {
    jQuery.ajax({
        dataType: 'json',
        url: '/eRSPG/committeemembers',
        success: (data) => {
        console.log(' fetchCommittee success!');
    committeeTableBuilder1(data);
},
    error: function() {
        console.error('fetchCommittee error!');
    },
